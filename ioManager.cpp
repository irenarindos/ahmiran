...
IOManager::IOManager( ) 
  ...
  : screen(SDL_SetVideoMode(Gamedata::getInstance().getXmlInt("screenWidth"),
                            Gamedata::getInstance().getXmlInt("screenHeight"), 
                            0, SDL_HWSURFACE)),
...
SDL_Surface* IOManager::loadAndSet(const string& filename, bool setalpha = true) const {
  ...
  if(setalpha){
    Uint32 alpha = SDL_MapRGBA(tmp->format, 0, 0, 0, 0);
    SDL_SetAlpha( tmp , SDL_RLEACCEL | SDL_SRCALPHA , alpha);
...
void IOManager::printMessageCenteredAt( const string& msg, Uint32 y) const {
   ...
   SDL_Surface *stext = TTF_RenderText_Blended(font, msg.c_str(), color);
   if (stext) {
     Uint32 x = ( Gamedata::getInstance().getXmlInt("screenWidth") - stext->w ) / 2;
     SDL_Rect dest = {x,y,0,0};
     SDL_BlitSurface( stext, NULL, screen, &dest );
     SDL_FreeSurface(stext);
   }
